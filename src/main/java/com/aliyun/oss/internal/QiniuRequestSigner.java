    ...
    private void signGetObject(RequestMessage request) throws ClientException {
        ...
        if (accessKeyId.length() > 0 && secretAccessKey.length() > 0) {
            ...
            try {
                ...
                request.addHeader(OSSHeaders.HOST, this.endPoint.getHost());
                getLog().warn("=========== hostA: " + this.endPoint.getHost());
                getLog().warn("=========== urlA: " + url.toString());
            } catch (MalformedURLException err) {
                getLog().warn("=========== signGetObject exception: " + err.getMessage());
            }
        }
        */
    }
